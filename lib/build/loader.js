...
exports.createLoaderCode = function createLoaderCode(platform, bundler) {
  ...
  switch (loaderOptions.type) {
    case 'require':
      let envPath = path.join(process.cwd(), 'aurelia_project/environments/' + CLIOptions.getEnvironment());
      let env  = require(envPath).default;
      let config = exports.createRequireJSConfig(platform, bundler);
      config.baseUrl = env.uiUrl + '/' + config.baseUrl;
      loaderCode = 'requirejs.config(' + JSON.stringify(config) + ')';
      break;
    case 'system':
