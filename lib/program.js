...
AureliaCLI.ready = function(argv, env, program) {
  ...
  isCommand('update') && program
  .command('update')
  .description('Update all aurelia packages to latest version')
  .option('--nuclear', "Wipe all package folders when updating")
  .action(commands('update'))
  .on('--help', function () {
    log('  Examples:');
    log();
    log('    $ aurelia update');
    log('    $ aurelia update -nuclear');
    log();
  });
