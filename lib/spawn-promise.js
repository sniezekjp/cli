...
function SpawnPromise(options) {
    ...
    return new Promise(function(resolve, reject){
        ...
        if ( !options ) {
            ...
            return reject('spawn Requires an options parameter!')
        }

        if ( !options.command ) {
            logger.err('spawn requires a command property on options!');
            return reject('spawn requires a command property on options!')
        }

        if ( options.args && !Array.isArray(options.args) ) {
            logger.err('options.args must be an array!');
            return reject('options.args must be an array!')
        }
